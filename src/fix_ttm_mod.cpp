...
FixTTM_MOD::FixTTM_MOD(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 9) error->all(FLERR,"Illegal fix ttm_mod command");
  ...
  if (nfileevery) {
    if (narg != 11) error->all(FLERR,"Illegal fix ttm_mod command");
    MPI_Comm_rank(world,&me);
    if (me == 0) {
      fp = fopen(arg[10],"w");
      if (fp == NULL) {
        char str[128];
        sprintf(str,"Cannot open fix ttm_mod file %s",arg[10]);
        error->one(FLERR,str);
      }
    }
  }
...
//t_surface is determined by electronic temperature (not constant)
  ...
  memory->create(nsum,nxnodes,nynodes,nznodes,"ttm_mod:nsum");
  memory->create(nsum_all,nxnodes,nynodes,nznodes,"ttm_mod:nsum_all");
  memory->create(T_initial_set,nxnodes,nynodes,nznodes,"ttm_mod:T_initial_set");
  memory->create(sum_vsq,nxnodes,nynodes,nznodes,"ttm_mod:sum_vsq");
  memory->create(sum_mass_vsq,nxnodes,nynodes,nznodes,"ttm_mod:sum_mass_vsq");
  memory->create(sum_vsq_all,nxnodes,nynodes,nznodes,"ttm_mod:sum_vsq_all");
  memory->create(sum_mass_vsq_all,nxnodes,nynodes,nznodes,
    		"ttm_mod:sum_mass_vsq_all");
  memory->create(T_electron_old,nxnodes,nynodes,nznodes,"ttm_mod:T_electron_old");
  memory->create(T_electron,nxnodes,nynodes,nznodes,"ttm_mod:T_electron");
  memory->create(net_energy_transfer,nxnodes,nynodes,nznodes,
    		"TTM_MOD:net_energy_transfer");
  memory->create(net_energy_transfer_all,nxnodes,nynodes,nznodes,
		"TTM_MOD:net_energy_transfer_all");
...
void FixTTM_MOD::setup(int vflag)
{
...
void FixTTM_MOD::post_force(int vflag)
{
  double **x = atom->x;
  double **v = atom->v;
  double **f = atom->f;
...
void FixTTM_MOD::post_force_setup(int vflag)
{
  double **f = atom->f;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixTTM_MOD::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixTTM_MOD::post_force_respa_setup(int vflag, int ilevel, int iloop)
{
...
void FixTTM_MOD::read_initial_electron_temperatures()
{
  char line[MAXLINE];
...
/* ---------------------------------------------------------------------- */
el_heat_capacity_thermal_conductivity FixTTM_MOD::el_properties(double T_e)
{
...
double FixTTM_MOD::el_sp_heat_integral(double T_e)
{
  double T_temp = T_e/1000.0, T_reduced = T_damp*T_temp;
  if (T_damp != 0)
...
double FixTTM_MOD::memory_usage()
{
  double bytes = 0.0;
...
void FixTTM_MOD::grow_arrays(int ngrow) 
{

 memory->grow(flangevin,ngrow,3,"TTM_MOD:flangevin");
...
double FixTTM_MOD::compute_vector(int n)
{
  double e_energy = 0.0;
...
void FixTTM_MOD::write_restart(FILE *fp)
{
  double *rlist;
  memory->create(rlist,nxnodes*nynodes*nznodes+1,"TTM_MOD:rlist");
...
void FixTTM_MOD::restart(char *buf)
{
  int n = 0;
...
int FixTTM_MOD::pack_restart(int i, double *buf)
{
  buf[0] = 4;
  buf[1] = flangevin[i][0];
  buf[2] = flangevin[i][1];
...
void FixTTM_MOD::unpack_restart(int nlocal, int nth)
{
...
int FixTTM_MOD::maxsize_restart()
{
  return 4;
}
...
int FixTTM_MOD::size_restart(int nlocal)
{
  return 4;
}
